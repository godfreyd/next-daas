...
const SITE_URL_REG = /^(https?:\/\/|\/\/)yandex\.ru/i;
// The headers will be updated to include a link to their hash
const HEADINGS = ['h2', 'h3', 'h4', 'h5', 'h6'];

function removeExt(path) {
  const basePath = path.split(/#|\?/)[0];
  const { dir, name } = parse(basePath);

  return join(dir, name) + path.substring(basePath.length);
}
...
export default function rehypeDocs({ filePath, tag }) {
  ...
  function visitAnchor(node) {
    ...
    props.href = href.replace(SITE_URL_REG, '');
    ...
    if (props.href === href) {
      ...
      if (isAbsoluteUrl || isRepoUrl) {
        ...
        if (!isAbsoluteUrl) {
          ...
          props.href = `${SITE_URL}` + resolve(filePath, href);
        }
        return;
      }
    }
